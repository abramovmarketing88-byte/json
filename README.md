...
5. Через 1–2 минуты приложение откроется по адресу:
   ...
   ```
   Пример: если репозиторий `telegram-json-splitter`, то  
   `https://username.github.io/telegram-json-splitter/`
...
В корне (или в той папке, которую вы выбрали как источник для Pages) должны лежать:

- `index.html`
- `index.tsx`

Остальное (README, `.gitignore` и т.д.) не обязательно для запуска.
